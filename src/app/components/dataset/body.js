...
export function UpdateBody({ dataset, session }) {
  ...
  const handleSubmit = async () => {
    ...
    let datasetFilePath = null;

    if(datasetFile)
      datasetFilePath = await uploadDataset(datasetFile);
    ...
    if(datasetFilePath)
    {
      obj['dataset'] = datasetFilePath
      obj['size'] = datasetFile.size
    }
    ...
    {
      toast({
        title: "Failed to update the dataset.",
        description: "Please try again..."
      });
    }
      ...
      toast({
        title: "Successfully updated your dataset!",
      });
    }
    ...
    <div className="w-full h-[100vh] overflow-y-scroll flex flex-col p-10 gap-10">
      ...
      <div className="w-full flex flex-row justify-center">
        <div className="w-full max-w-[400px] flex flex-col gap-5">
          ...
          <textarea value={description} placeholder="description... (you can paste markdown here)" className="border-[0.5px] min-h-[100px] rounded-md p-2 outline-none" onChange={(e)=>setDescription(e.target.value)} />
        </div>
      </div>
